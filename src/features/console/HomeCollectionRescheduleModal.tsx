...
export function HomeCollectionRescheduleModal({
  ...
  const handleCheckServiceability = async () => {
    if (!addressLine.trim()) {
      setStep1Error("Address is required.");
      return;
    }
    if (!city.trim()) {
    ...
    if (!pincode.trim()) {
      setStep1Error("Pincode is required.");
      return;
    }
  ...
  const handleReschedule = async () => {
    ...
    try {
      const res = await consoleApi.rescheduleHomeCollection(engagementId, participant.user_id, {
        blood_collection_date: selectedDate,
        blood_collection_time_slot_id: selectedSlot.stm_id ?? "",
        blood_collection_time_slot: selectedSlot.slot_time ?? "",
      ...
      onRescheduled({
        ...
        slot_start_time: data.slot_start_time ?? selectedSlot.slot_time ?? null,
      });
    } catch (err) {
      setRescheduleError(getApiError(err));
    } finally {
  ...
  return (
    <Modal
      open={open && mayEditConsole}
      onClose={handleClose}
      title="Reschedule Home Collection"
      ...
      <div className="space-y-5">
        ...
        {step === 4 && (
          <div className="space-y-4 text-center py-4">
            ...
            <div>
              <h4 className="text-lg font-medium text-zinc-900">Booking Rescheduled</h4>
              <p className="text-sm text-zinc-600 mt-1">
                Home collection has been rescheduled successfully.
              </p>
            </div>
